...
class Bbcode
{
    ...
    public function parse(?string $source, bool $replaceLineBreaks = true): string
    {
            ...
            if ($source[$index + 1] === '/' && !empty($openedElements)) {
                ...
                if (strcasecmp($tag, (string) $el['closeBbcode']) === 0) {
                    if (\array_key_exists('callback', $el)) {
                        ...
                        $source = substr_replace($source, $replacement, $expectedEl['tagStart'], $length);
                ...
                foreach (self::parsers() as $name => $el) {
                    ...
                    if (preg_match($el['openBbcode'], $remainingText, $matches) === 1) {
                        ...
                        if ($name === 'list-item' && ($openedElements === [] || !\in_array(end($openedElements)['name'], ['unorderedlist', 'orderedlistalpha', 'orderedlistnumerical']))) {
                            $source = substr_replace($source, '[list]', $index, 0);
                            preg_match('/(\r\n|\n|\r)\h*(\r\n|\n|\r)/', $source, $endListMatches, PREG_OFFSET_CAPTURE, $index);
                            $endListIndex = $matches == [] ? \strlen($source) : $endListMatches[0][1];
                            $source = substr_replace($source, '[/list]', $endListIndex, 0);
                            $index--;

                            break;
                        }

                        if (!\array_key_exists('callback', $el) && ($openedElements === [] || self::parsers()[end($openedElements)['name']]['parseWithin'])) {
                        ...
                        if ($el['closeBbcode'] !== null && ($openedElements === [] || self::parsers()[end($openedElements)['name']]['parseWithin'])) {
                            $openedElements[] = [
                                'name'     => $name,
                                'tagStart' => $index,
                            ];
        ...
        while (!empty($openedElements)) {
            $source .= self::parsers()[array_pop($openedElements)['name']]['closeHtml'];
        }
