<?php

declare(strict_types=1);

/**
 * NOTICE OF LICENSE.
 *
 * UNIT3D Community Edition is open-sourced software licensed under the GNU Affero General Public License v3.0
 * The details is bundled with this project in the file LICENSE.txt.
 *
 * @project    UNIT3D Community Edition
 *
 * @author     HDVinnie <hdinnovations@protonmail.com>
 * @license    https://www.gnu.org/licenses/agpl-3.0.en.html/ GNU Affero General Public License v3.0
 */

return [
    'activation-error'        => '帳戶已封禁 或 確認碼無效 或 已經確認',
    'activation-success'      => '確認成功，現在可登入!',
    'add-image'               => '增加一項',
    'add-profile'             => '增加一項',
    'allow-invite'            => '自由註冊已關閉!你只能透過朋友的邀請才能註冊!',
    'allow-invite-appl'       => '自由註冊已關閉!你只能透過朋友的邀請或提交申請表格才能註冊!',
    'appl-closed'             => '申請已關閉',
    'appl-intro'              => '是一個封閉社群. 若你沒被邀請，只能提交申請表等待審批。',
    'appl-reason'             => '你如何得知 :sitename ? 為何要加入? ',
    'application'             => 'Application',
    'application-submitted'   => '申請已提交。若審批通過，你會收到電郵通知。',
    'apply'                   => 'Apply',
    'are-you'                 => '你是:',
    'banned'                  => '此帳戶已封!',
    'check-later'             => '稍後再來!',
    'code'                    => 'TOTP Code',
    'confirm-new-password'    => 'Confirm New Password',
    'confirm-password'        => 'Confirm Password',
    'delete-image'            => '減少一項',
    'delete-profile'          => '減少一項',
    'email'                   => '郵箱',
    'email-verification-link' => 'An email verification link has been emailed to you!',
    'enter-recovery'          => 'Please enter one of the recovery codes provided to you when first enabling two-factor authentication.',
    'enter-totp'              => 'Please enter the 6-digit authentication code generated by your app.',
    'failed'                  => '身份憑證與站點記錄不符合。',
    'invalid-key'             => '無效邀請碼!',
    'login'                   => '登入',
    'login-now-on'            => '立即登入',
    'logout'                  => '登出',
    'lost-password'           => '忘了密碼？',
    'lost-username'           => '忘了用戶名？',
    'need-invite'             => '開放註冊已關閉，請從已註冊用戶處獲得邀請!',
    'new-password'            => 'New Password',
    'newbie'                  => '小萌新',
    'not-a-member'            => '還不是成員？30秒內便能完成註冊。',
    'not-activated'           => '此帳戶仍未啟用，請檢查郵箱，若遺失郵件，請點擊忘記密碼重新獲取。',
    'password'                => '密碼',
    'password-confirm-desc'   => 'Please confirm your password before proceeding.',
    'password-confirmation'   => 'Password confirmation',
    'password-reset'          => 'Reset Password',
    'proof-image'             => '它站截圖連結',
    'proof-image-title'       => 'Links to profile screenshots',
    'proof-min'               => '(最少2, 建議3)',
    'proof-profile'           => '它站個人檔案連結',
    'proof-profile-title'     => 'Links to your profiles',
    'recover-my-password'     => '重設密碼',
    'recovery-code'           => 'Recovery code',
    'register-thanks'         => '感謝註冊，請檢查郵箱。',
    'remember-me'             => '記住我的登錄',
    'require-rules'           => '請閱讀頁底的條款並接納。',
    'send-verification-email' => 'Send Verification Email',
    'signup'                  => '註冊',
    'throttle'                => '登錄次數過多。請 :seconds 秒後再試。',
    'totp-code'               => 'TOTP code',
    'unlock'                  => '解鎖',
    'user-icon'               => '用戶頭像',
    'username'                => '用戶名',
    'verify'                  => 'Verify',
    'verify-email'            => 'Verify Email',
    'verifying'               => 'Verifying...',
    'veteran'                 => '老司機',
    'welcome'                 => '歡迎回來!',
    'welcome-restore'         => '歡迎回來!你的帳戶已解封!'
];
